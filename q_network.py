...
class QCodingNetwork(object):
    ...
    def train_epoch(self, x_batches, y_batches):
        ...
        for batch_id, (x_batch, y_batch) in enumerate(zip(x_batches, y_batches)):
            ...
            """ TODO where do we include the predictions? top-down pass
            x[0] = y_batch
            for l in range(1, self.n_layers):
            ...
            self.update_params(x, q, errors, batch_size)
    ...
    def test_amortised(self, x_batches, y_batches):
        accs = []
        for x_batch, y_batch in zip(x_batches, y_batches):
            x_batch = set_tensor(x_batch, self.device)
            y_batch = set_tensor(y_batch, self.device)
    ...
    def update_params(self, x, q, errors, batch_size):
        q = q[::-1]
        ...
        self._apply_gradients(grad_w, grad_b, grad_w_q, grad_b_q)
    ...
    def _apply_gradients(self, grad_w, grad_b, grad_w_q, grad_b_q):
        ...
        if self.optim is "RMSPROP":
            ...
            for l in range(self.n_layers - 1):
                ...
                self.W_q[l] = self.W_q[l] + self.l_rate * grad_w_q[l].T
                self.b_q[l] = self.b_q[l] + self.l_rate * grad_b_q[l].unsqueeze(dim=1)
