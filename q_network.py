...
class QCodingNetwork(object):
    ...
    def test_epoch(self, x_batches, y_batches, itr_max=None):
        ...
        for x_batch, y_batch in zip(x_batches, y_batches):
            ...
            x, errors, _ = self.infer_v2(x, batch_size, x_batch, itr_max=itr_max)
            pred_y = x[0]
    ...
    def infer_v2(self, x, batch_size, x_batch, itr_max=None):
        ...
        for itr in range(itr_max):
            # TODO
            """
            fn_deriv = F.f_deriv(torch.matmul(x[0], self.W[0].T), self.act_fn)
            errors[0] = (x[1] - x[0])  self.vars[0]
            delta = torch.matmul(errors[0] * fn_deriv, self.weights)
            x[0] = x[l] + beta  * delta
            """
