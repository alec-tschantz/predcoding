...
class QCodingNetwork(object):
    ...
    def hybrid_infer(self, x, batch_size, itr_max=None, test=False):
        ...
        q_errors = [[] for _ in range(self.n_layers)]
        q_f_x_arr = [[] for _ in range(self.n_layers)]
        q_f_x_deriv_arr = [[] for _ in range(self.n_layers)]
        ...
        for l in range(1, self.n_layers):
            # bottom up
            x = x[::-1]
            q_f_x = F.f(x[l - 1], self.act_fn)
            q_f_x_deriv = F.f_deriv(x[l - 1], self.act_fn)
            q_f_x_arr[l - 1] = q_f_x
            q_f_x_deriv_arr[l - 1] = q_f_x_deriv

            q_b = self.b_q[l - 1].repeat(1, batch_size)
            q_errors[l] = (x[l] - self.W_q[l - 1] @ q_f_x - q_b) / self.vars[l]
            q_f_0 = q_f_0 - self.vars[l] * torch.sum(torch.mul(q_errors[l], q_errors[l]), dim=0)
        ...
        for itr in range(itr_max):
            ...
            for l in range(1, self.n_layers):
                ...
                q_errors[l] = (x[l] - self.W_q[l - 1] @ q_f_x - self.b_q[l - 1]) / self.vars[l]
                q_f = q_f - self.vars[l] * torch.sum(torch.mul(q_errors[l], q_errors[l]), dim=0)
